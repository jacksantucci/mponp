...
load("../data/brooktrans.Rdata")

### for seeing what parties are in the data
lapply(brooktrans, function(x) unique(x$Party))

unique(unlist(lapply(brooktrans, function(x) x$Party)))
...
getLeakage <- function(transfers.in){
	...
	## coding coalitions here
	
	# nondominant.parties <- c("A", "F", "C", "L", "R", "R-F", "R-CNP", "R-F-CNP", "F-CNP", "F-CNP", "F-C-NP", "D-A")
	
	# nondominant.parties <- c("A", "C", "D-A")
...
foo <- lapply(brooktrans, getLeakage)
