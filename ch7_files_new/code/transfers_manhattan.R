...
load("../data/mantrans.Rdata")

### for seeing what parties are in the data
unique(unlist(lapply(mantrans, function(x) x$Party)))
...
foo <- lapply(mantrans, getLeakage)
...
### isaacs, connolly in 1945

m45 <- transferReport(mantrans[[5]], 3)

isaacs45 <- lapply(m45, function(x) x[x$Candidate=="IsaacsSM",])

i45 <- do.call(rbind, isaacs45)

i45tot <- aggregate(rawChange ~ Party.send, data=i45, FUN="sum")
i45tot$pct <- i45tot$rawChange/sum(i45tot$rawChange)

connolly45 <- lapply(m45, function(x) x[x$Candidate=="ConnollyEP",])

c45 <- do.call(rbind, connolly45)

c45tot <- aggregate(rawChange ~ Party.send, data=c45, FUN="sum")
c45tot$pct <- c45tot$rawChange/sum(c45tot$rawChange)
